...
local GetPlayersString = function()
...
function Library:MakeDraggable(Instance, Cutoff)
	...
	Instance.InputBegan:Connect(function(Input)
		...
		elseif Input.UserInputType == Enum.UserInputType.Touch then
			...
			local ObjPos = Vector2.new(InputPosition.X - Instance.AbsolutePosition.X, InputPosition.Y - Instance.AbsolutePosition.Y)
			if ObjPos.Y > (Cutoff or 40) then
				return
			end
			local Connection
			Connection = InputService.InputChanged:Connect(function(ChangedInput)
				if ChangedInput.UserInputType == Enum.UserInputType.Touch and ChangedInput == Input then
					local CurrentPosition = ChangedInput.Position
					Instance.Position = UDim2.new(0, CurrentPosition.X - ObjPos.X + (Instance.Size.X.Offset * Instance.AnchorPoint.X), 0, CurrentPosition.Y - ObjPos.Y + (Instance.Size.Y.Offset * Instance.AnchorPoint.Y))
				end
			end)
			local EndConnection
			EndConnection = InputService.InputEnded:Connect(function(EndedInput)
				if EndedInput.UserInputType == Enum.UserInputType.Touch and EndedInput == Input then
...
do
	...
	function Funcs:AddButton(...)
		local Button = {}
		local ProcessButtonParams = function(Class, Obj, ...)
		...
		local Groupbox = self
		local Container = Groupbox.Container
		local CreateBaseButton = function(Button)
		...
		local InitEvents = function(Button)
			local WaitForEvent = function(event, timeout, validator)
			...
			local ValidateClick = function(Input)
				if Library:MouseIsOverOpenedFrame() then
					return false
				end
	...
	function Funcs:AddSlider(Idx, Info)
		...
		SliderInner.InputBegan:Connect(function(Input)
			if (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch) and not Library:MouseIsOverOpenedFrame() then
				...
				local GuiPosition = Fill.Size.X.Offset
				local Difference = MousePosition - (Fill.AbsolutePosition.X + GuiPosition)
				local IsDragging = true
				local UpdateSlider = function()
					local NewMousePosition
					if Input.UserInputType == Enum.UserInputType.Touch then
						NewMousePosition = Input.Position.X
					else
						NewMousePosition = Mouse.X
					end
					local NewX = math.clamp(GuiPosition + (NewMousePosition - MousePosition) + Difference, 0, Slider.MaxSize)
				...
				if Input.UserInputType == Enum.UserInputType.Touch then
					...
					InputEnded = Input.Ended:Connect(function()
						IsDragging = false
	...
	function Funcs:AddDropdown(Idx, Info)
		...
		local RecalculateListSize = function(YSize)
